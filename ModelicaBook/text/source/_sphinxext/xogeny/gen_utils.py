...
def findModel(*frags):
    rfrags = [re.compile(x) for x in frags]
    res = []
    ...
    for plot in plots:
        ...
        if pdata["type"]=="simple":
            with open(os.path.join(path, "text", "results",
                ...
                obj = dict(pdata)
                obj["vars"] = [x.dict() for x in pdata["vars"]]
...
def _generate_makefile():
    ...
    for res in results:
        ...
        if len(mods)==0:
            ...
            simflags = "-cpu -ignoreHideResult -emit_protected -override "+(",".join([str(x)+"="+str(mods[x]) for x in mods]))
